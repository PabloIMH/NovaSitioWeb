...
from .forms import CustomUserCreationForm, ArchivoForm
from .models import PerfilUsuario, Archivo, ArchivoItem
...
class ArchivoItemInline(admin.TabularInline):
    model = ArchivoItem
    extra = 1  # Número de formularios en blanco adicionales a mostrar

class ArchivoAdmin(admin.ModelAdmin):
    form = ArchivoForm  # Usar solo el formulario que no incluye el archivo
    inlines = [ArchivoItemInline]  # Mover carga de archivos a ArchivoItemInline
    list_display = ('titulo', 'usuario_asignado', 'fecha_subida')
    search_fields = ('titulo', 'usuario_asignado__username')
    list_filter = ('usuario_asignado', 'fecha_subida')
